...
本地关键词库渠道（`spam.local`）固定从 `configs/spam/keywords.txt` 读取词库文件，数据库与
管理 API 不传输词库内容，也不接受词库路径：

- 文件必须是服务端可读的普通 UTF-8 文本文件；每行一个关键词或短语，忽略空行、
  去首尾空白并去重，Unicode 大小写不敏感匹配。
- 大小限制：单文件不超过 64 MiB，单行不超过 4000 字节；超出则整次加载失败并保留
  最近成功快照。
- 容器部署需把宿主机词库绑定到容器内的 `/app/configs/spam/keywords.txt`；启用
  `spam.local` 前请先创建或挂载该文件。
- 运行期按文件大小与修改时间热重载，词库变更无需重启；保存配置时会立即校验文件。
