...
const (
	...
	// CallbackFormPost 回调参数以 application/x-www-form-urlencoded POST 传递。
	CallbackFormPost CallbackMode = "form_post"
)

// ConfigSpec 固定 provider 的配置 schema。
type ConfigSpec struct {
	// PublicFields 允许进入 public_config 的字段（JSON key）。
	PublicFields []string
	// SecretField 机密字段的 JSON key（client_secret 或 private_key）；空表示该 provider 无机密。
	SecretField string
	// InstanceURLRequired 要求管理端显式提供 instance_url。
	InstanceURLRequired bool
	// InstanceURLDefault instance_url 留空时使用的默认值；空表示无默认值。
	InstanceURLDefault string
	// FixedPublic 预设锁定的公开字段值（如 github 的 auth_url/token_url、google 的 issuer_url），
	// 管理端提交的同名字段会被覆盖。
	FixedPublic map[string]string
}

// ProviderSpec 固定内置 OAuth/OIDC provider 的元数据。
type ProviderSpec struct {
	// Key 固定 provider key，每种内置 provider 只允许一条配置。
	Key string
	// Name 展示名。
	Name string
...
// Catalog 固定内置 provider 的只读目录，按 key 升序。
